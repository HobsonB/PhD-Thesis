...
# Runs parallelized EnergyPlus simulations for all ten occupancy scenarios using eplusr with adjacent zone control
...
# Load in distance matrix for zone associations
m <- as.matrix(read.csv("./euc_mx.csv",row.names=1))
# Vector of Vp values
Vp <- as.vector(t(read.csv("./Vp.csv")))
# Vector of room areas
A <- as.vector(t(read.csv("./rm_area.csv")))
...
run_simulations <- function(input_datum) {
  ...
  # Find which random zones have sensors for controlling adjacent zones
  rand_zone <- which(rand_sensor %in% c(2,3))
  # Create a vector with the index of the controlling zone for each of the 26 zones
  adj_zones <- rm_nam[rand_zone[Rfast::rowMins(as.matrix(m[,rand_zone]), value = F)]]
  ...
  idf <- Idf$new(paste0(idf_path,"/model_euclidean.idf"))
  ...
  for(i in c(1:length(rm_nam))){
    # Changing to appropriate TrimResFun
    temp_idf$replace_value(paste0("#fun",rm_nam[i],"#"),paste0("TrimResFun_",rand_sensor[i]))
    # Voz
    temp_idf$replace_value(paste0("#Voz",rm_nam[i],"#"),paste0("Voz",adj_zones[i]))
    # Vp
    temp_idf$replace_value(paste0("#Vp",rm_nam[i],"#"),paste0(Vp[which(adj_zones[i] == rm_nam)]))
    # co2
    temp_idf$replace_value(paste0("#co2",rm_nam[i],"#"),paste0("co2",adj_zones[i]))
